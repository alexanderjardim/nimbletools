...
const App: React.FC = () => {
  ...
  const [resumeData, setResumeData] = useState<{ characterData: Record<string, any>, currentStep: number } | undefined>(undefined);
  ...
  const handleResumeCharacterClick = () => {
    ...
    if (inProgressData) {
      console.log('handleResumeCharacterClick: Setting resumeData to inProgressData');
      setResumeData(inProgressData);
      console.log('handleResumeCharacterClick: Navigating to CharacterCreation');
      setCurrentPage('CharacterCreation');
    }
  };

  const handleCharacterSaved = () => {
    console.log('handleCharacterSaved: Character saved, going back to Characters page');
