...
export class BackgroundDataLoader {
    ...
    public readonly backgrounds: ReadonlyMap<string, Background> = this._backgrounds;
     ...
     */
    public static create(backgroundData: IBackgroundData[]): BackgroundDataLoader {
        ...
        for (const rawData of backgroundData) {
            ...
            if (!name || !rawData.description) {
                throw new Error(`Background data is missing required properties for name "${name}"`);
            }
        }
