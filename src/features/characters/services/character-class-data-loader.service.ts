...
export class CharacterClassDataLoader {
    ...
    public readonly characterClasses: ReadonlyMap<string, CharacterClass> = this._characterClasses;
     ...
     */
    public static create(characterClassesData: ICharacterClassData[]): CharacterClassDataLoader {

        const loader = CharacterClassDataLoader.instance || new CharacterClassDataLoader();
    ...
    private processCharacterClasses(data: ICharacterClassData[]) {
        for (const rawData of data) {
            ...
            const hitDice = new HitDice(rawData.hitDice.dice, rawData.hitDice.quantity);

            const characterClass = new CharacterClass(
                rawData.id, rawData.name, rawData.image, rawData.teasers,
                rawData.hitPoints, hitDice, savesMap
            );
