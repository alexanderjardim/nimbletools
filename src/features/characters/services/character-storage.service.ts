export class CharacterStorageService {
     ...
     */
    public static saveInProgressCharacter(characterData: Record<string, any>, currentStep: number): void {
        try {
            console.log('CharacterStorageService saveInProgressCharacter called:', { characterData, currentStep });
            // Don't save if the data is essentially empty
            const hasProgress = characterData.class || characterData.ancestry || characterData.background ||
                Object.keys(characterData.stats || {}).length > 0 ||
                Object.keys(characterData.skills || {}).length > 0 ||
                characterData.name || currentStep > 1;

            console.log('CharacterStorageService hasProgress check:', hasProgress);

            if (!hasProgress) {
                console.log('CharacterStorageService clearing in-progress data (no progress)');
            ...
            localStorage.setItem(this.IN_PROGRESS_KEY, JSON.stringify(inProgressData));
     ...
     */
    public static getInProgressCharacter(): { characterData: Record<string, any>, currentStep: number } | null {
        try {
