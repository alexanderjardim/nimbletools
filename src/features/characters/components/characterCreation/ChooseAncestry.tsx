...
const ChooseAncestry: React.FC<ChooseAncestryProps> = ({ onNext, onBack }) => {
    ...
    const handleNextStep = () => {
        ...
        if (selectedAncestry) {
            console.log('ChooseAncestry calling onNext with:', { ancestryId: selectedAncestry });
    ...
    if (isLoading) {
        return <div className="text-ink text-center p-10">Loading ancestries...</div>;
    }

    return (
        <div className="p-8 font-sans bg-parchment min-h-full">
            <h1 className="text-4xl font-bold text-center text-ink mb-2">Choose Your Ancestry</h1>
            <p className="text-center text-chestnut mb-6">Select an ancestry that defines your character's heritage.</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {ancestries.map(ancestry => (
                    ...
                    >
                        ...
                        />
                        <div className="p-6">
                            <h3 className="text-2xl font-bold text-ink mb-3">{ancestry.name}</h3>
