...
const CharacterReview: React.FC<CharacterReviewProps> = ({
  ...
  const selectedClass = characterData.class
    ? classLoader.characterClasses.get(characterData.class)
    : null;
  const selectedAncestry = characterData.ancestry
    ? ancestryLoader.ancestries.get(characterData.ancestry)
    : null;
  const selectedBackground = characterData.background
    ? backgroundLoader.backgrounds.get(characterData.background)
    : null;
  ...
  console.log("CharacterReview Debug:", {
    ...
    backgroundExists: backgroundLoader.backgrounds.has(
      characterData.background || "",
    ),
    allBackgroundIds: Array.from(backgroundLoader.backgrounds.keys()),
    allBackgroundNames: Array.from(backgroundLoader.backgrounds.values()).map(
      (bg) => bg.name,
    ),
  });
  ...
  return (
    <div className="p-8 font-sans bg-parchment min-h-full">
      ...
      <div className="text-center mb-6">
        <h2 className="text-3xl font-bold text-bronze mb-2">
          {characterData.name || "Unnamed Character"}
        </h2>
        <h3 className="text-xl text-ink mb-2">
          Level 1 • {selectedClass?.name || "No Class"} •{" "}
          {selectedAncestry?.name || "No Ancestry"} •{" "}
      ...
      <p className="text-center text-chestnut mb-8">
        Review your character details before creating. Click on any section to
        make changes.
      </p>
      ...
      <div className="max-w-4xl mx-auto space-y-6">
          ...
          {selectedClass ? (
            <div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div>
                  <label className="block text-chestnut text-sm mb-1">
                    Hit Points
                  </label>
                ...
                <div>
                  ...
                  <p className="text-ink">
                    {selectedClass.hitDice.quantity}d
                    {selectedClass.hitDice.dice}
                  </p>
                </div>
              </div>
              <div className="mb-4">
                <label className="block text-chestnut text-sm mb-2">
                  Class Features
                </label>
                <ul className="text-ink space-y-1">
                  {selectedClass.teasers.map(
                    (teaser: string, index: number) => (
        ...
        <div className="bg-gradient-to-br from-parchment-light to-parchment p-6 rounded-lg border-2 border-bronze shadow-lg">
          ...
          {selectedAncestry ? (
            <div>
              <h3 className="text-xl text-bronze mb-2">
        ...
        <div className="bg-parchment-light p-6 rounded-lg border-2 border-saddle-brown">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {characterData.stats &&
              Object.entries(characterData.stats).map(
                ([stat, value]: [string, any]) => (
                  <div key={stat} className="text-center">
                    <div className="text-2xl font-bold text-bronze">
                      {value}
                    </div>
                    <div className="text-chestnut text-sm uppercase">
                      {stat}
                    </div>
                  </div>
                ),
        ...
        <div className="bg-parchment-light p-6 rounded-lg border-2 border-saddle-brown">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {characterData.skills &&
              Object.entries(characterData.skills).map(
        ...
        {selectedClass && (
          <div className="bg-parchment-light p-6 rounded-lg border-2 border-saddle-brown">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {Object.entries(selectedClass.saves).map(
                ([stat, saveType]: [string, any]) => (
                  <div key={stat} className="text-center">
                    <div className="text-chestnut capitalize">{stat}</div>
                    <div
                      className={`text-sm ${saveType === "Advantaged" ? "text-green-600" : saveType === "Disadvantaged" ? "text-red-600" : "text-bronze"}`}
                    >
                      {saveType}
                    </div>
                  </div>
