...
import { SkillName, SKILL_STATS, SKILL_NAMES } from '../../../../shared/constants/character.constants';
...
const AssignSkills: React.FC<AssignSkillsProps> = ({ onNext, onBack }) => {
    ...
    return (
        <div className="p-8 font-sans bg-parchment min-h-full">
            <h1 className="text-4xl font-bold text-center text-ink mb-2">Assign Skill Points</h1>
            <p className="text-center text-chestnut mb-10">Distribute 4 skill points across your skills.</p>
            ...
            <div className="bg-parchment-light p-6 rounded-lg max-w-md mx-auto border-2 border-saddle-brown">
                ...
                <div className="mt-4 text-center">
                    <p className={`text-sm ${canProceed() ? 'text-green-600' : 'text-red-600'}`}>
                        Points Used: {calculateTotalPoints()} / {totalPointsAvailable}
                    </p>
                    {!canProceed() && (
                        <p className="text-xs text-red-500 mt-1">
                            You must assign all {totalPointsAvailable} skill points before continuing
                        </p>
                    )}
                </div>
            </div>
