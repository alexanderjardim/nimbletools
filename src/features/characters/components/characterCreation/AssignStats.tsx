...
const AssignStats: React.FC<AssignStatsProps> = ({ onNext, onBack }) => {
    ...
    const handleStatChange = (stat: string, value: number) => {
        ...
        const statWithValue = Object.entries(stats).find(([_, statValue]) => statValue === value);
    ...
    return (
        <div className="p-8 font-sans bg-parchment min-h-full">
            <h1 className="text-4xl font-bold text-center text-ink mb-2">Assign Your Stats</h1>
            <p className="text-center text-chestnut mb-10">Choose a stat array and assign modifiers to each stat.</p>

            <div className="mb-6 flex justify-center items-center space-x-4">
                <div className="flex items-center space-x-4">
                    <label htmlFor="statArray" className="text-ink font-medium">Stat Array:</label>
                    <select
                        id="statArray"
                        value={selectedArray}
                        onChange={(e) => handleArrayChange(e.target.value as keyof typeof statArrays)}
            ...
            <div className="max-w-2xl mx-auto">
                <div className="bg-parchment-light p-6 rounded-lg border-2 border-saddle-brown">
                    <h3 className="text-2xl font-bold text-ink mb-6">Stat Distribution</h3>
                        ...
                        {STAT_NAMES.map(stat => {
                            ...
                            return (
                                <div key={stat} className="flex items-center space-x-4">
                                    <label className="text-ink capitalize font-medium min-w-0 w-32">{stat}</label>
                                    <select
                                        value={currentValue}
