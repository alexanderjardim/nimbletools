...
const CharactersPage: React.FC<CharactersPageProps> = ({ onBackClick, onCreateNewCharacter, onResumeCharacter }) => {
    ...
    const loadCharacters = () => {
        ...
        try {
            ...
            const characterList: Character[] = Object.entries(savedCharacters).map(([id, data]) => ({
                id,
    ...
    const handleDeleteCharacter = (characterId: string) => {
        ...
        if (isEmptyName || window.confirm('Are you sure you want to delete this character?')) {
            try {
    ...
    return (
        <div className="w-screen flex-shrink-0 p-6 box-border overflow-y-auto flex flex-col items-center justify-between bg-parchment font-fell">
            <div className="w-full flex-grow flex flex-col items-center">
                <h2 className="text-4xl text-saddle-brown mb-4 text-shadow-md">Characters</h2>

                {isLoading ? (
                    <p className="text-brown-dark text-center mt-8">Loading characters...</p>
                ) : characters.length === 0 ? (
                    <div className="text-center text-brown-dark py-8 flex flex-col items-center gap-4">
                        ...
                        {hasInProgressCharacter && (
                            <button className={primaryButtonStyles} onClick={handleResumeCharacter}>
                                Resume Character Creation
                            </button>
                        )}
                        <button className={primaryButtonStyles} onClick={handleCreateNewCharacter}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 py-6 w-full max-w-6xl">
                        {characters.map((character) => (
                            <div key={character.id} className="bg-parchment-light rounded-lg p-6 border-2 border-saddle-brown shadow-lg transition-all duration-300 hover:border-bronze hover:shadow-glow-red flex flex-col">
                                <div className="mb-4">
                                    <h3 className="text-2xl font-bold text-brown-dark mb-2">{character.name}</h3>
                                    <div className="text-brown-dark text-sm">
                                        <p className="capitalize">Level 1 {character.ancestry} {character.class}</p>
                                        <p className="capitalize text-saddle-brown">{character.background}</p>
                                    </div>
                                </div>
                                ...
                                <div className="mb-4">
                                    ...
                                    <div className="grid grid-cols-2 gap-2 text-sm">
                                        <div className="text-brown-dark">STR: <span className="font-bold">{character.stats.strength}</span></div>
                                        <div className="text-brown-dark">DEX: <span className="font-bold">{character.stats.dexterity}</span></div>
                                        <div className="text-brown-dark">INT: <span className="font-bold">{character.stats.intelligence}</span></div>
                                        <div className="text-brown-dark">WILL: <span className="font-bold">{character.stats.will}</span></div>
                                    </div>
                                </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 mt-auto pt-6 w-full justify-center max-w-lg">
                <button className={secondaryButtonStyles} onClick={onBackClick}>Back to Menu</button>
