...
describe('Character', () => {
    ...
    it('should construct a character with correct base properties', () => {
        ...
        const character = new Character('Elara', 1, mockCharacterClass, mockAncestry, mockBackground, stats, skillPoints);
    ...
    it('should correctly initialize stats from the stats map', () => {
        const stats = new Map([['strength', 0], ['dexterity', 2], ['intelligence', 4], ['will', 1]]);
        const skillPoints = new Map<string, number>();
        const character = new Character('Elara', 1, mockCharacterClass, mockAncestry, mockBackground, stats, skillPoints);
    ...
    it('should default a stat to 0 if not provided in the map', () => {
        const stats = new Map([['strength', 0], ['intelligence', 4]]);
        const skillPoints = new Map<string, number>();
        const character = new Character('Elara', 1, mockCharacterClass, mockAncestry, mockBackground, stats, skillPoints);
    ...
    it('should correctly calculate skill totals (stat + points)', () => {
        const stats = new Map([['strength', 1], ['dexterity', 3], ['intelligence', 5], ['will', 2]]);
        const skillPoints = new Map([['arcana', 2], ['might', 3], ['stealth', 1]]);
        const character = new Character('Elara', 1, mockCharacterClass, mockAncestry, mockBackground, stats, skillPoints);
    ...
    it('should throw an error if a calculated skill total exceeds 12', () => {
        ...
        const createCharacter = () => new Character('Elara', 1, mockCharacterClass, mockAncestry, mockBackground, stats, skillPoints);

        expect(createCharacter).toThrow('Skill arcana total (stat + points) cannot exceed 12.');
    });
});