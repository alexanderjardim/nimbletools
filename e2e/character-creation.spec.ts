...
test.describe('Character Creation Flow - Google Pixel 7', () => {
    test.beforeEach(async ({ page }) => {
        ...
        const createButton = page.getByRole('button', { name: 'Create New Character', exact: true });
        if (await createButton.isVisible()) {
            // Already on characters page, proceed directly
            await createButton.click();
        } else {
            // Need to navigate to characters page first
            const charactersButton = page.getByRole('button', { name: 'Characters', exact: true });
    ...
    test('should complete full character creation, save character, and verify CSS styling on Google Pixel 7', async ({ page }) => {
        ...
        await expect(page.locator('div.p-8.font-sans.bg-parchment.min-h-full')).toBeVisible();
        await expect(page.getByRole('heading', { name: 'Choose Your Class' })).toHaveClass(/text-ink/);
        await expect(page.getByText('Select a class to begin your adventure.')).toHaveClass(/text-chestnut/);
        ...
        await expect(page.locator('div.p-8.font-sans.bg-parchment.min-h-full')).toBeVisible();
        await expect(page.getByRole('heading', { name: 'Choose Your Ancestry' })).toHaveClass(/text-ink/);
        ...
        await expect(page.locator('div.p-8.font-sans.bg-parchment.min-h-full')).toBeVisible();
        await expect(page.getByRole('heading', { name: 'Choose Your Background' })).toHaveClass(/text-ink/);
        ...
        await expect(page.getByRole('heading', { name: 'Review Character' })).toBeVisible();
        // Note: Character name is randomly generated by the random fill button, so we just verify a name exists
        await expect(page.locator('h2.text-3xl.font-bold.text-bronze.mb-2')).toBeVisible();
        ...
        await expect(page.locator('div.p-8.font-sans.bg-parchment.min-h-full')).toBeVisible();
        await expect(page.getByRole('heading', { name: 'Review Character' })).toHaveClass(/text-ink/);
    ...
    test('should verify CSS styling consistency across all components', async ({ page }) => {
